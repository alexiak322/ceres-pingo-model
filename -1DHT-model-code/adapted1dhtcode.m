...
nLob=[0.33 0.5 0.66];    % [min interm. max] Lobate
...
dz=2;               % cell size [m]
grid_depth=600;     % grid depth, GRID SPANS 600m (depth of lobate flow)
z=0:dz:grid_depth;  % cell nodes, 0,2,4,6 M ETC.
...
T_ann10q=xT_ann10q;     % These temperatures are used in the model for-loop, MEAN ANNUAL AIR TEMPERATURE DURING EACH TIME STEP FOR 11000 YR
...
% 1=L
onegrid=ones(1,nocell); % 1 x 301 matrix
                % Material IDs ZONEs 2-3 (3), 1-2 (2), 0-1 (1)
Z0_1MatID=ones(1,nocell); % divvying up what portion of the column is dominated by which material, 0-58m = quaternary
Z1_2MatID=Z0_1MatID;  % not really sure what's going on here—each zone = 1000 years? tracks how materials changed?
Z2_3MatID=Z0_1MatID;
...
n_grid=onegrid; % all of these have 501 elements -> all material properties define at nodes (1 x 501 matrix)
p_soil=onegrid;
cp_soil=onegrid;
k_soil=onegrid;

F_w(1)=0;         % TOTAL FRACTION OF WATER
F_w(nocell)=1;    % all elements have 0 percent water except for bottom
f_w=onegrid;      % FRACTION OF LIQUID WATER IN THE PORE SPACE
f_w(1)=0;
F_ice(1)=1;
F_ice(nocell)=0;
f_ice(nocell)=0;

dF_w=zeros(1,(nocell-1)); % CHANGE IN TOTAL FRACTION OF WATER??? — 500 ELEMENTS
k_eq=zeros(1,(nocell-1)); % EQUIVALENT THERMAL CONDUCTIVITY — 500 ELEMENTS
C_eq=zeros(1,(nocell));   % EQUIVALENT HEAT CAPACITY — 501 ELEMENTS
a_eq=zeros(1,(nocell));   % EQUIVALENT THERMAL DIFFUSIVITY — 501 ELEMENTS

T_matx_0_1=zeros(nocell,500); % CREATING EMPTY TEMP MATRICES FOR EACH ZONE'S RUN TIME, 500 yr
T_matx_1_2=zeros(nocell,1500); % 501 x 1500 matrix (r x c), 100 yr, EACH YEAR HAS CORRESPONDING TEMP FOR EACH CELL
T_matx_2_3=zeros(nocell,2500); % 2500 yr, etc.
...
F_matx_0_1=zeros(nocell,500); % NOT SURE WHAT F MEANS, IS THIS FRACTION OF WATER OF EACH CELL IN EACH ZONE?
...
for col=col_incl
  ...
  for ii=1:nocell       %%% Assigning thermal parameters to column, cell nodes? %%%
    if materialid(ii)==1     % only one material; might change to account for carapace     %%% Porosity
...
no_tstep=runtime/tstep; % Total number of time step, DIFFERENT FOR EACH ZONE
...
stability=(max(a_s_ice)*tstep/dz^2);
 ...
 %%%  Heat transfer loops begins here  %%%
    if col==1       %%% 1st column begins %%%
        xT_ini=T_ini;
        k=0;     %%% time step count (for zone 1: l = 10,000)
        l=0;     %%% year count (for zone 1: l = 500 yr)
        for t=1:no_tstep
            if stability>0.5
                continue
            end
                ...
                for i=2:(nocell-1) % 2 TO 500 (excluding top and bottom bc they have prescribed fractions)
                        ...
                        elseif xT_ini(i)>0
                            ...
                            dF_w(i)=-2*xT_ini(i)*exp(-(xT_ini(i)/w)^2); %%% diff of eq. A
                              ...
                              %%% Total fractions of w and ice, POPULATING
                              %%% EACH CELL WITH TOTAL FRACTIONS OF W AND
                              %%% ICE
                        f_w(i)=F_w(i)*n_grid(i); % N_GRID IS A 501 COLUMN MATRIX OF ALL ONES
                    ...
                    if (xT_ini(i+1)-xT_ini(i-1))~=0 % NOT SURE WHAT THIS IS SAYING
                        T(t,i)=(a_eq(i)*tstep/dz^2)*(xT_ini(i+1)+xT_ini(i-1))+((1-(2*a_eq(i)*tstep/dz^2))*xT_ini(i));
                    else
                        T(t,i) = xT_ini(i);  % REMINDER: T IS A 1X501 MATRIX t = time step, i = cell
                    end
                              %%% End-grid temperatures %%%
                    T(t,1)=T_1(t); % TOP, Holocene surface temperature for given time
                    T(t,nocell)=T(t,(nocell-1))+dz*T_gradient; % BOTTOM, Holocene temp + (2m*temp gradient)
                end      
  ...
  %%% 11th column ends %%% 
    ...
    fprintf('complete')               %%% 12th column ends %%% 
    end
end
toc
%%                          %%% Figures %%%
...
%%              %%%  Final equiv. vol. heat capacity distribution
%%
if any(col_incl==1)
figure(12)   %%% 1st column - Zone 0-1
plot(C_eq_0_1(1,2:100), z(1,2:100),'+'); % x axis has 99 columns, y axis spans from 2–198 m and has 501 columns
title('Volumetric heat capacity - Zone 0-1')
xlabel('Equivalent heat capacity [J/ºC]')
ylabel('Depth [m]')
set(gca,'YDir', 'reverse')
grid on
movegui('west')
...
if any(col_incl==1)
    ...
    ylabel('Depth [m]')
    movegui('center')
    legend('Freezing front', 'Permafrost front')
